...
test('PUT /api/Complaints/123', { signal: AbortSignal.timeout(timeout) }, async () => {
  await runner
    .createScan({
      tests: ['bopla', 'file_upload', 'xss', 'sqli', 'csrf'],
    ...
    .run({
      ...
      url: `${baseUrl}/api/Complaints/123`,
      body: {
        message: "This is a sample complaint message.",
        file: "optional-file-path-or-url"
      },
